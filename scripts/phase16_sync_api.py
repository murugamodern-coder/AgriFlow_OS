...
"""Sync API with sync_correlation_id in envelope (Phase 16)."""
...
def pull(data=None):
	try:
		...
		return success(body, include_sync_meta=True)
	except frappe.AuthenticationError as exc:
		return fail("AUTH_REQUIRED", str(exc), http_status=401, include_sync_meta=True)
	except frappe.ValidationError as exc:
		return fail("VAL_INVALID", str(exc), http_status=400, include_sync_meta=True)
	except frappe.PermissionError as exc:
		return fail("PERM_DENIED", str(exc), http_status=403, include_sync_meta=True)
...
def push(data=None):
	try:
		...
		if not operations:
			return fail("VAL_INVALID", _("operations required"), http_status=400, include_sync_meta=True)
		...
		if outcome.get("partial"):
			...
			envelope = success(data_out, include_sync_meta=True)
			envelope["error"] = {
				"code": SYNC_PARTIAL,
			...
			return envelope
		return success(data_out, include_sync_meta=True)
	except frappe.AuthenticationError as exc:
		return fail("AUTH_REQUIRED", str(exc), http_status=401, include_sync_meta=True)
	except frappe.ValidationError as exc:
		return fail("VAL_INVALID", str(exc), http_status=400, include_sync_meta=True)
